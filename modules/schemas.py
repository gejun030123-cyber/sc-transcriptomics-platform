...
SC_MODULE_LIST = [
    ...
    {'name': 'deg', 'display': '簇 Marker（探索性）', 'desc': 'cluster-versus-rest marker；细胞不是生物学重复'},
    {'name': 'sc_cell_deg', 'display': '细胞级探索性比较', 'desc': '同一样本不同簇、不同样本同一簇或条件间的描述性细胞级比较'},
    {'name': 'sc_pseudobulk_deg', 'display': '样本级 pseudobulk 差异表达', 'desc': '以生物学样本为重复的正式条件比较；默认只呈现图和统计审计'},
...
PARAM_SCHEMAS = {
    ...
    'sc_cell_deg': [
        ...
        {'key': 'selected_cluster', 'label': '选定 cluster', 'type': 'text', 'default': '', 'show_if': {'comparison_type': 'between_samples_within_cluster'}, 'help': '填写 cluster 的精确值；随后 comparisons 填 SampleA-vs-SampleB。'},
        {'key': 'analysis_scope', 'label': '条件比较范围', 'type': 'select', 'options': ['both', 'all_cells', 'per_cluster'], 'default': 'both', 'show_if': {'comparison_type': 'condition'}, 'help': 'both：整体和逐簇条件比较；per_cluster：每个簇内比较同一条件对。'},
    ...
    'sc_cell_go': [
        ...
        {'key': 'target_clusters', 'label': '运行的 cluster（可选）', 'type': 'dynamic_multiselect', 'depends_on': 'deg_source_task_id', 'default': '', 'help': '先选择 DEG 来源任务，再勾选要运行富集的 cluster。仅勾选一个即可只运行该簇；留空则对该 DEG 任务的全部 cluster 运行。All 表示全体细胞/全体细胞类型的 DEG 范围，不是某个具体 cluster。'},
        ...
        {'key': 'plot_top_n', 'label': '每张富集图展示 term 数', 'type': 'number', 'default': 12, 'min': 1, 'max': 30, 'step': 1, 'help': '仅控制图中展示数量，CSV 结果仍按 top_n 输出。'},
        {'key': 'plot_max_clusters', 'label': '每个比较最多绘制 cluster 数（0 = 全部）', 'type': 'number', 'default': 0, 'min': 0, 'step': 1, 'help': '默认 0：对每个已完成且有显著通路的 cluster 分别生成富集图。填写正整数时，按最小 FDR 优先仅绘制前 N 个簇；完整富集统计表始终包含全部簇。'},
    ...
    'functional_state': [
        ...
        {'key': 'min_gene_set_genes', 'label': '一般通路最少检测基因数', 'type': 'number', 'default': 5, 'min': 2, 'step': 1, 'help': '一般自定义/内置通路少于此数跳过。FA import、FA transport、Ketogenesis 是已知小型签名，至少命中 2 基因仍保留但会告警。'},
        ...
        {'key': 'heatmap_features', 'label': '热图展示特征与顺序（可选）', 'type': 'textarea', 'default': '', 'help': '逗号、分号或换行分隔。按填写顺序筛选并排列热图行，例如：PPARA activity、Mitochondrial beta-oxidation score、Peroxisomal FAO score、HALLMARK_OXPHOS score。留空展示所有已计算的 TF activity 与 pathway score；热图以“细胞类型分组标题 + 条件子列”排版，避免长标签重叠。仅影响图，不影响完整表、统计或 FDR。'},
        {'key': 'correlation_x', 'label': '相关性 X 特征', 'type': 'select', 'options': ['PPARA activity', 'HNF4A activity', 'ESRRA activity', 'PPARG activity', 'RELA activity', 'STAT3 activity', 'Curated PPARA-target module score', '__custom__'], 'option_labels': {'__custom__': '自定义已计算特征'}, 'default': 'PPARA activity', 'help': '默认是真正由 CollecTRI + ULM 计算的 PPARA activity。Curated PPARA-target module score 是 expression module，绝不是 TF activity；二者不可互换。选择的 TF 未通过 coverage 或未计算时，相关性会明确标为不可用。'},
        ...
        {'key': 'correlation_y', 'label': '相关性 Y 特征', 'type': 'select', 'options': ['Inflammatory response score', 'TNF-NFkB response score', 'IFN response score', 'Curated PPARA-target module score', 'Mitochondrial beta-oxidation score', 'Peroxisomal FAO score', '__custom__'], 'option_labels': {'__custom__': '自定义已计算特征'}, 'default': 'Inflammatory response score', 'help': '默认使用炎症 pathway score；可切换到其它预设或自定义已计算通路。'},
        ...
        {'key': 'concordance_pathway_features', 'label': '一致性矩阵通路评分（可选）', 'type': 'textarea', 'default': 'FA import score,Mitochondrial beta-oxidation score,Peroxisomal FAO score,Inflammatory response score,TNF-NFkB response score,IFN response score', 'help': '用于验证 TF 与功能通路的方向一致性；例如 PPARA–FAO、RELA–TNF-NFkB、STAT1–IFN。平台同时输出 pooled 与 ctrl/dis 分层矩阵；均基于 sample × celltype 聚合，仅作重复测量探索性效应量展示，不显示显著性星号或 P/FDR。'},
    ...
    'sc_pseudobulk_deg': [
        ...
        {'key': 'plot_max_clusters', 'label': '每个比较最多绘制 cluster 数（0 = 全部）', 'type': 'number', 'default': 0, 'min': 0, 'step': 1, 'show_if': {'analysis_scope': 'per_cluster'}, 'help': '默认 0：为每个已完成的 cluster 条件比较分别生成 Volcano 和 MA 图。填写正整数时，按显著 DEG 数优先仅绘制前 N 个簇。'},
    ...
    'bulk_enrichment': [
        ...
        {'key': 'databases', 'label': '批量基因集数据库', 'type': 'static_multiselect',
         ...
         'help': '推荐 GO 三本体 + KEGG + Reactome；WikiPathways 可按研究问题补充。每个库保留独立完整结果表，概览仅用于浏览，不能把不同数据库的 FDR 合并解释。'},
