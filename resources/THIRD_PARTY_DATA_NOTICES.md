...
| WikiPathways Human | `WikiPathways_2024_Human`; `gene_sets/wikipathways/wikipathways_human_2024_enrichr.gmt` | WikiPathways 内容为 [CC0 1.0](https://www.wikipathways.org/terms.html)；应署名 WikiPathways，并按 Enrichr 的要求引用其基因集服务 | [Enrichr download](https://maayanlab.cloud/Enrichr/geneSetLibrary?mode=text&libraryName=WikiPathways_2024_Human) |
...
通过管理员维护流程完成，并先复核对应许可证；不要把 KEGG 或任何来源不明的本地 `genesets/`
文件直接加入仓库。上表 WikiPathways 快照是已记录来源与校验值的例外，不应以未记录的本地副本覆盖。
